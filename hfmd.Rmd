...
test_degrees <- function(yr, degree) {
  dataset <- filter(sero, year == as.character(yr))
  model <- glm(make_formula(degree), binomial, dataset)
  lrt(model)
}
```
...
```{r}
marc <- filter(sero, year == as.character(2022))
m1 <- glm(make_formula(5), binomial, marc)

marc <- filter(sero, year == as.character(2023))
m2 <- glm(make_formula(5), binomial, marc)
...
```{r}
fff <- function(yr = 2022) {
  marc <- filter(sero, year == as.character(yr))
  m1 <- glm(make_formula(5), binomial, marc)
  lrt(m1)
}
